...
from services.curadoria_comercial import (
    CuradoriaComercial
)
...
class PontuadorOferta:
    ...
    def calcular(
        ...
        pontos_desconto = (
            self._calcular_pontos_desconto(
                oferta
            )
        )
        ...
        pontos_historico = (
            self._calcular_pontos_historico(
                resultado_historico
            )
        )

        resultado_comercial = (
            self.curadoria_comercial.analisar(
                oferta
            )
        )

        pontos_comerciais = min(
            resultado_comercial.nota,
            self.PONTOS_MAXIMOS_COMERCIAL
        )
        ...
        oferta.motivos_comerciais = list(
            resultado_comercial.motivos
        )
    ...
    def _calcular_pontos_desconto(
        ...
        desconto_limitado = min(
            desconto,
            self.DESCONTO_PARA_PONTUACAO_MAXIMA
        )
    ...
    def _calcular_pontos_historico(
        ...
        return min(
            pontos_queda + pontos_menor_preco,
            15.0
        )
