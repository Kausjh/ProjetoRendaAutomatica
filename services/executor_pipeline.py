...
class ExecutorPipeline:
    ...
    async def executar(self) -> None:
        ...
        logger.info(
            "Iniciando nova execução do ProjetoRendaAutomatica."
        )

        logger.info(
            "Quantidade de scrapers configurados: %s",
            self.quantidade_scrapers
        )

        logger.info(
            "Limite de ofertas por scraper: %s",
            self.limite_ofertas
        )

        logger.info(
            "Máximo de publicações por execução: %s",
            self.maximo_publicacoes
        )

        logger.info(
            "Intervalo entre publicações: %s segundo(s)",
            self.intervalo_publicacoes
        )

        try:
            ofertas = self.coletor.buscar_ofertas(
                limite_por_scraper=self.limite_ofertas
            )
        ...
        logger.info(
            "Quantidade total de ofertas encontradas: %s",
            len(ofertas)
        )
        ...
        for oferta in ofertas:
            ...
            try:
                resultado_historico = (
                    self.historico_precos_service.analisar_e_registrar(
                        oferta
                    )
                )
                ...
                logger.exception(
                    "Erro ao analisar o histórico de preço de '%s'.",
                    oferta.nome
                )

                quantidade_com_erro += 1

            resultado_filtro = self.filtro.analisar(
                oferta
            )

            if not resultado_filtro.aprovada:
                logger.info(
                    "Oferta rejeitada: %s | Motivo: %s",
                    oferta.nome,
                    resultado_filtro.motivo
                )
            ...
            oferta_ja_publicada = self.repository.ja_foi_publicada(
                oferta.link
            )
            ...
            if (
                oferta_ja_publicada
                and not deve_republicar_por_queda
            ):
                logger.info(
                    "Ignorando oferta já publicada: %s",
                    oferta.nome
                )
            ...
            logger.info(
                "Oferta elegível: %s | Pontuação final: %.2f",
                oferta.nome,
                pontuacao
            )
        ...
        ofertas_selecionadas = ofertas_aprovadas[
            :self.maximo_publicacoes
        ]

        quantidade_nao_selecionada = (
            len(ofertas_aprovadas)
            - len(ofertas_selecionadas)
        )

        logger.info(
            "Ofertas aprovadas pelo filtro: %s",
            quantidade_aprovada_pelo_filtro
        )

        logger.info(
            "Ofertas elegíveis para publicação: %s",
            len(ofertas_aprovadas)
        )

        logger.info(
            "Ofertas selecionadas para publicação: %s",
            len(ofertas_selecionadas)
        )

        if quantidade_nao_selecionada > 0:
            logger.info(
                "Ofertas elegíveis deixadas para outra execução: %s",
                quantidade_nao_selecionada
            )
        ...
        for (
            ...
            logger.info(
                "Publicando: %s | Pontuação final: %.2f",
                oferta.nome,
                pontuacao
            )
            ...
            try:
                ...
                self.repository.marcar_como_publicada(
                    oferta.link
                )
                ...
                if deve_republicar_por_queda:
                    ...
                    logger.info(
                        "Oferta republicada por queda de preço: %s",
                        oferta.nome
                    )
        ...
        logger.info(
            "Taxa de aprovação do filtro: %.2f%%",
            taxa_aprovacao_filtro
        )
        ...
        logger.info(
            "Ofertas republicadas por queda de preço: %s",
            quantidade_republicada_por_queda
        )

        logger.info(
            "Ofertas elegíveis não selecionadas: %s",
            quantidade_nao_selecionada
        )

        logger.info(
            "Ofertas rejeitadas pelo filtro: %s",
            quantidade_filtrada
        )

        logger.info(
            "Ofertas já publicadas e sem nova queda: %s",
            quantidade_ignorada
        )

        logger.info(
            "Novos preços registrados: %s",
            quantidade_precos_registrados
        )

        logger.info(
            "Quedas de preço detectadas: %s",
            quantidade_quedas_detectadas
        )

        logger.info(
            "Novos menores preços históricos: %s",
            quantidade_menores_precos
        )

        logger.info(
            "Links de publicação processados: %s",
            quantidade_links_processados
        )

        logger.info(
            "Links transformados: %s",
            quantidade_links_transformados
        )

        logger.info(
            "Links mantidos sem alteração: %s",
            quantidade_links_mantidos
        )

        if afiliadores_utilizados:
            logger.info(
                "Afiliadores utilizados:"
            )

            for nome_afiliador, quantidade in (
                afiliadores_utilizados.items()
            ):
                logger.info(
                    "- %s: %s",
                    nome_afiliador,
        ...
        logger.info(
            "Tempo total da execução: %.2f segundo(s)",
            tempo_total
        )
        ...
        relatorio = {
            ...
            "ofertas_aprovadas_pelo_filtro": (
                quantidade_aprovada_pelo_filtro
            ),
            "taxa_aprovacao_filtro": round(
                taxa_aprovacao_filtro,
                2
            ),
            "ofertas_elegiveis": len(
                ofertas_aprovadas
            ),
            "ofertas_selecionadas": len(
                ofertas_selecionadas
            ),
            "ofertas_publicadas": quantidade_publicada,
            "ofertas_republicadas_por_queda": (
                quantidade_republicada_por_queda
            ),
            "ofertas_elegiveis_nao_selecionadas": (
                quantidade_nao_selecionada
            ),
            "ofertas_rejeitadas": quantidade_filtrada,
            "ofertas_ja_publicadas_sem_nova_queda": (
                quantidade_ignorada
            ),
            "novos_precos_registrados": (
                quantidade_precos_registrados
            ),
            "quedas_preco_detectadas": (
                quantidade_quedas_detectadas
            ),
            "novos_menores_precos_historicos": (
                quantidade_menores_precos
            ),
            "links_processados": quantidade_links_processados,
            "links_transformados": quantidade_links_transformados,
            "links_mantidos_sem_alteracao": (
                quantidade_links_mantidos
            ),
        ...
        self.relatorios_repository.salvar(
            relatorio
        )
