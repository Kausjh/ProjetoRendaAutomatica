    ...
    def formatar_resumo(self) -> str:
        ...
        return "\n".join(
            [
                ...
                ("Preços antigos removidos: " f"{self.precos_antigos_removidos}"),
                ("Descontos divergentes: " f"{self.descontos_divergentes}"),
                ("Descontos fora do intervalo: " f"{self.descontos_fora_intervalo}"),
                ("Descontos acima do limite confiável: " f"{self.descontos_acima_limite}"),
                separador,
            ]
        )
...
class ValidadorOferta:
    ...
    def __init__(
        self, tolerancia_desconto: float = 2.0, desconto_maximo_confiavel: float = 90.0
    ) -> None:
        if tolerancia_desconto < 0:
            raise ValueError("A tolerância de desconto não pode ser negativa.")

        if not 0 < desconto_maximo_confiavel <= 100:
            raise ValueError("O desconto máximo confiável deve estar entre 0 e 100.")

        self.tolerancia_desconto = tolerancia_desconto
        self.desconto_maximo_confiavel = desconto_maximo_confiavel

    def validar(self, oferta: Oferta, estatisticas: EstatisticasValidacao | None = None) -> Oferta:
        oferta.valida = True
        ...
        self._validar_dados_obrigatorios(oferta, estatisticas)

        self._normalizar_desconto_anunciado(oferta, estatisticas)

        self._validar_preco_antigo(oferta, estatisticas)
    ...
    def _validar_dados_obrigatorios(
        self, oferta: Oferta, estatisticas: EstatisticasValidacao | None
    ) -> None:
        if not oferta.nome.strip():
            if estatisticas is not None:
                estatisticas.nomes_vazios += 1

            self._invalidar(oferta, "Nome da oferta vazio.")

        if oferta.preco <= 0:
            if estatisticas is not None:
                estatisticas.precos_invalidos += 1

            self._invalidar(oferta, "Preço atual menor ou igual a zero.")

        if not self._link_valido(oferta.link):
            if estatisticas is not None:
                estatisticas.links_invalidos += 1

            self._invalidar(oferta, "Link da oferta inválido.")

    @staticmethod
    def _normalizar_desconto_anunciado(
        oferta: Oferta, estatisticas: EstatisticasValidacao | None
    ) -> None:
        ...
        if desconto < 0 or desconto > 100:
            ...
            ValidadorOferta._adicionar_motivo(
                oferta, "Desconto anunciado fora do intervalo de 0% a 100%."
            )
        ...
        oferta.desconto_anunciado = round(desconto, 2)
    ...
    def _validar_preco_antigo(
        ...
        if preco_antigo <= 0:
            self._remover_preco_antigo(oferta, "Preço antigo menor ou igual a zero.", estatisticas)
            return

        if preco_antigo <= oferta.preco:
            self._remover_preco_antigo(
                oferta, "Preço antigo menor ou igual ao preço atual.", estatisticas
            )
            return
        ...
        if desconto_calculado > self.desconto_maximo_confiavel:
            ...
            self._remover_preco_antigo(
                oferta,
                ("Desconto calculado acima do limite confiável: " f"{desconto_calculado:.2f}%."),
                estatisticas,
            )
            return
        ...
        divergencia = abs(desconto_calculado - desconto_anunciado)
    ...
    def _link_valido(link: str) -> bool:
        ...
        resultado = urlparse(link.strip())

        return resultado.scheme in {"http", "https"} and bool(resultado.netloc)

    @staticmethod
    def _invalidar(oferta: Oferta, motivo: str) -> None:
        oferta.valida = False

        ValidadorOferta._adicionar_motivo(oferta, motivo)

        logger.warning("Oferta marcada como inválida: '%s' | Motivo: %s", oferta.nome, motivo)

    @staticmethod
    def _remover_preco_antigo(
        oferta: Oferta, motivo: str, estatisticas: EstatisticasValidacao | None
    ) -> None:
        ...
        logger.debug(("Preço antigo removido da oferta '%s'. " "Motivo: %s"), oferta.nome, motivo)

    @staticmethod
    def _adicionar_motivo(oferta: Oferta, motivo: str) -> None:
        if motivo not in oferta.motivos_validacao:
            oferta.motivos_validacao.append(motivo)
