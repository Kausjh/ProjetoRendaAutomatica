...
from affiliates.registro_afiliadores import (
...
from repositories.publicados_repository import (
    PublicadosRepository
)
...
async def main() -> None:
    try:
        ...
        logger.exception(
            "Erro nas configurações do projeto."
        )
    ...
    logger.info(
        "Desconto mínimo configurado: %s%%",
        configuracoes.desconto_minimo
    )

    logger.info(
        "Preço máximo configurado: %s",
        configuracoes.preco_maximo
    )

    logger.info(
        "Identificador público da marca: %s",
        configuracoes.identificador_marca
    )

    logger.info(
        "Nicho configurado: hardware, periféricos "
        "e produtos gamer."
    )
