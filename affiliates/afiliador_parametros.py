...
class AfiliadorParametros(BaseAfiliador):
    ...
    def __init__(self, nome: str, dominios: list[str], parametros: dict[str, str]) -> None:
        nome_normalizado = nome.strip()

        if not nome_normalizado:
            raise ValueError("O nome do afiliador não pode ficar vazio.")

        if not dominios:
            raise ValueError("O afiliador precisa possuir pelo menos um domínio.")
        ...
        if not self.dominios:
            raise ValueError("O afiliador precisa possuir pelo menos um domínio válido.")
    ...
    def suporta(self, link: str) -> bool:
        dominio_link = urlparse(link).netloc.lower()
    ...
    def gerar_link(self, link_original: str) -> str:
        partes_link = urlparse(link_original)

        parametros_existentes = dict(parse_qsl(partes_link.query, keep_blank_values=True))

        parametros_existentes.update(self.parametros)

        nova_query = urlencode(parametros_existentes)

        return urlunparse(partes_link._replace(query=nova_query))
