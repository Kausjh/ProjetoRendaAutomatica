...
class OfertaFilter:
    ...
    def __init__(
        self, desconto_minimo: float, preco_maximo: float, relevancia_nicho_minima: float = 55
    ) -> None:
        self.desconto_minimo = desconto_minimo
        self.preco_maximo = preco_maximo
        self.relevancia_nicho_minima = relevancia_nicho_minima

    def analisar(self, oferta: Oferta) -> ResultadoFiltro:
        if not oferta.eh_nicho:
            return ResultadoFiltro(
                aprovada=False,
                motivo=("Oferta não pertence ao nicho de " "hardware e produtos gamer."),
            )

        if oferta.relevancia_nicho < self.relevancia_nicho_minima:
            return ResultadoFiltro(
                aprovada=False,
                motivo=(
        ...
        if oferta.preco <= 0:
            return ResultadoFiltro(
                aprovada=False, motivo="O preço atual precisa ser maior que zero."
            )
        ...
        return ResultadoFiltro(
            aprovada=True, motivo=("Oferta aprovada pelos critérios de nicho, " "preço e desconto.")
        )
